...
{
    // This requires Postgres database to be setup.
    ...
    public class PostgreSqlCacheWithDatabaseTest
    {
        ...
        public PostgreSqlCacheWithDatabaseTest()
        {
            ...
            {
                { ConnectionStringKey, "Server=localhost;Port=5432;Database=CacheTestDb;User Id=postgres;Password=postgres;" },
        ...
        private PostgreSqlCache GetCache(ISystemClock testClock = null)
        {
            ...
            return new PostgreSqlCache(new TestPostgreSqlCacheOptions(options));
        }

        private async Task AssertGetCacheItemFromDatabaseAsync(
            PostgreSqlCache cache,
            string key,
            byte[] expectedValue,
        ...
        private async Task<CacheItemInfo> GetCacheItemFromDatabaseAsync(string key)
        {
            using (var connection = new NpgsqlConnection(_connectionString))
            {
                ...
                if (await reader.ReadAsync())
                {
                    ...
                    {
                        cacheItemInfo.SlidingExpirationInSeconds = TimeSpan.FromSeconds(reader.GetDouble(3));
                    }
